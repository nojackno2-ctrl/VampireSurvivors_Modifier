...
public sealed class ProcessMemorySession : IProtectedMemoryAccessor, IDisposable
{
    ...
    public ProcessModuleInfo GetModuleInfo(string moduleName)
    {
        ...
        foreach (ProcessModule module in _process.Modules)
        {
            using (module)
            {
                ...
                {
                    return new ProcessModuleInfo(module.BaseAddress, module.ModuleMemorySize);
                }
            }
        }
    ...
    public byte[] ReadBytes(nint address, int length)
    {
        ...
        if (!NativeMethods.ReadProcessMemory(_handle, address, buffer, (nuint)length, out nuint bytesRead)
    ...
    private void WriteCore(nint address, ReadOnlySpan<byte> bytes)
    {
        ...
        if (!NativeMethods.WriteProcessMemory(_handle, address, buffer, (nuint)buffer.Length, out nuint bytesWritten)
